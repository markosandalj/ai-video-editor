...
export function ResizableSplit({
  ...
  const sidebarStyle: CSSProperties | undefined = isDesktop
    ? { width: mainHidden ? '100%' : (width ?? SIDEBAR_DEFAULT_WIDTH) }
    : undefined
  ...
  return (
    <div ref={containerRef} className="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
      {!sidebarHidden && (
        <div
          className={cn(
            ...
            mainHidden ? 'min-w-0 flex-1' : 'shrink-0',
            !mainHidden && 'border-b md:border-b-0 md:border-r',
          )}
          style={sidebarStyle}
        >
          {sidebar}
      ...
      {!mainHidden && <div className="flex min-h-0 min-w-0 flex-1 flex-col">{main}</div>}
    </div>
  )
}

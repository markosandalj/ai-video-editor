...
export function subtractRange(
	...
	for (const r of ranges) {
		...
		if (hole.start > r.start + EPS)
			out.push({ start: r.start, end: hole.start });
...
function audibleSpan(word: Span): { start: number; end: number } {
	const start =
		word.cut_in != null ? Math.max(word.start, word.cut_in) : word.start;
	const end =
		word.cut_out != null ? Math.min(word.end, word.cut_out) : word.end;
	const audible =
		end > start ? { start, end } : { start: word.start, end: word.end };
...
export function sameRanges(a: TimeRange[], b: TimeRange[]): boolean {
	...
	for (let i = 0; i < a.length; i++) {
		if (
			Math.abs(a[i].start - b[i].start) > EPS ||
			Math.abs(a[i].end - b[i].end) > EPS
		)
			return false;
	}
	return true;

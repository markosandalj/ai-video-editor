...
from ai_video_editor.experiments.manifest import ExperimentRunConfig, load_manifest
...
def run_experiments(
    ...
    *,
    ...
    for run in manifest.runs:
        ...
        result = _run_cutting(run, model_config, fixture_names, fixtures_dir, output_dir)
        result.runtime_s = time.perf_counter() - started
        results.runs.append(result)

    results.completed_at = datetime.now(timezone.utc).isoformat()
    (output_dir / "results.json").write_text(results.model_dump_json(indent=2), encoding="utf-8")
    (output_dir / "report.md").write_text(format_report(results), encoding="utf-8")
    return results
...
def _run_cutting(
    ...
    settings = settings.model_copy(
        update={
            "section_editor": settings.section_editor.model_copy(update={"llm": model_config})
        }
    )
    ...
    for name in fixture_names:
        ...
        try:
            ...
            flags = detect_all_flags(raw, silences, [], settings)
            edl = build_edl(raw, keeps, flags, rescue_outside_keep_regions=False)
...
def _load_fixture(
    fixtures_dir: Path,
    name: str,
) -> tuple[Transcript, EditDecisionList, Transcript]:
    ...
    return (
        Transcript.model_validate_json(raw_path.read_text(encoding="utf-8")),
        EditDecisionList.model_validate_json(edl_path.read_text(encoding="utf-8")),
        Transcript.model_validate_json(gt_path.read_text(encoding="utf-8")),
    )


def format_report(results: ExperimentResults) -> str:
    lines = [
        "# Cutting Model Evaluation Report",
        "",
    ...
    for run in results.runs:
        ...
        error_count = len(run.fixtures) - ok_count
        lines.extend(
            [
                f"## {run.id}",
                "",
                ...
                "",
                *_format_cutting_summary(run.aggregate),
                "",
            ]
        )
        ...
        if failed:
            lines.extend(["### Errors", ""])
            lines.extend(f"- `{fixture.name}`: {fixture.error}" for fixture in failed[:20])
            lines.append("")
    return "\n".join(lines).rstrip() + "\n"
